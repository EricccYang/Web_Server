...
void http_handle_header(http_request_t* r, http_out_t* o) {
    ...
    list_for_each(pos, &(r->list));
    {
        ...
        for (header_in = http_header_in; strlen(header_in->name) > 0; header_in++) {
...
int http_process_if_modified_since(http_request_t* r, http_out_t* out, char* data, int len){
    (void) r;
    (void) len;

    struct tm tm;
//    if(strftime_l(data, "%a, %d, %b %Y, %H: %M:%S GMT", &tm) == (char*)NULL){
//        return OK;
//    }
